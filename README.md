...
一个可在本机或 Linux 服务器运行的雨课堂课堂监听与 LLM 辅助答题面板。它支持多账号扫码登录，在用户通过雨课堂 App 手动签到后监听课堂事件，从课件中解析题目，调用 OpenAI 兼容模型生成答案，并可按题目截止时间选择是否自动提交。
...
- 设置页可选择雨课堂、荷塘雨课堂、长江雨课堂或黄河雨课堂，默认长江雨课堂
...
| 组件 | 职责 |
| --- | --- |
| `static/index.html` | 本地控制面板、账号登录、模型配置与实时日志 |
...
```text
扫码登录 -> 雨课堂 App 手动签到 -> 连接课堂 WebSocket -> 同步课件 -> 解析题目
...
不要把应用的 `--port` 设置为 `443`。公网只需开放 `80/443`，不应开放后端端口 `8765`。
...
此模式默认监听 `0.0.0.0:8765`，通过 `http://服务器IP:8765` 访问且不提供传输加密，不建议直接暴露到公网。
...
1. 在“雨课堂服务器”中选择学校使用的部署；默认是长江雨课堂。
2. 在“模型配置”中填写 API 地址和 API Key，点击“获取模型”，选择模型后保存。
3. 点击“添加账号”，使用微信扫码登录。
4. 在雨课堂 App 中手动完成签到。
5. 保持“自动答题并提交”关闭，先通过日志确认课程、题目解析和模型返回均正常。
6. 只有在你确认有权这样做时，才开启自动提交。
...
| 设置 | 说明 |
| --- | --- |
| `server` | 雨课堂部署，可选 `yuketang`、`pro`、`changjiang`、`huanghe`，默认 `changjiang` |
...
```bash
python -m unittest -v test_ykt_web.py test_email_notifier.py test_deploy_script.py
```
